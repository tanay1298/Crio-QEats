...
def get_tags_suggestions(api_key, image_url):
    ...
    data =     {      "inputs": [       {          "data": {            "image": {              "url": f"{image_url}"           }          }        }      ]    }

    response = requests.post('https://api.clarifai.com/v2/models/bd367be194cf45149e75f01d59f77ba7/outputs', headers=headers, json=data).json()
